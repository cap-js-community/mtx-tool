...
describe("reg tests", () => {
  ...
  test("reg update tenant", async () => {
    ...
    expect(await reg.registryUpdateDependencies(await freshContext(), [testTenantId], [false])).toMatchInlineSnapshot(`
      "{
        "id": "4520b25d-29fc-4bbd-9403-4a76acebbfcd",
        "state": "SUCCEEDED"
      }"
    `);
    expect(outputFromLoggerPartitionFetch(loggerSpy.info.mock.calls)).toMatchInlineSnapshot(`
  ...
  test("reg update tenant all", async () => {
    ...
    expect(await reg.registryUpdateAllDependencies(await freshContext(), undefined, [false])).toMatchInlineSnapshot(`
      [
        "{
        "id": "a6334e8e-6b69-46c6-97cc-94a02c4965e5",
        "state": "SUCCEEDED"
      }",
        "{
        "id": "eba2a96a-3de7-4f75-a3cf-af6821d0c826",
        "state": "SUCCEEDED"
      }",
        "{
        "id": "ef9575a0-d3f9-4347-b792-24d60756bcdc",
        "state": "SUCCEEDED"
      }",
        "{
        "id": "cf60da54-382d-48dd-bd5a-0e4eae14ceb7",
        "state": "SUCCEEDED"
      }",
        "{
        "id": "17889327-5c21-4056-8d7c-ac75ede93088",
        "state": "SUCCEEDED"
      }",
        "{
        "id": "a8d43a70-80f4-4ed5-bf1f-0cb6b9e51463",
        "state": "SUCCEEDED"
      }",
        "{
        "id": "bb2c6cc7-6432-4def-b2fa-e97375268e00",
        "state": "SUCCEEDED"
      }",
        "{
        "id": "d47be903-50f9-425b-94e1-b464d987087f",
        "state": "SUCCEEDED"
      }",
        "{
        "id": "cd27bfbc-3f18-4297-a24c-cbc92035b80d",
        "state": "SUCCEEDED"
      }",
        "{
        "id": "78ff106a-3a26-4246-a4db-6da81cecc0fa",
        "state": "SUCCEEDED"
      }",
      ]
    `);
  ...
  test("reg update tenant application url all", async () => {
    ...
    expect(outputFromLoggerPartitionFetch(loggerSpy.info.mock.calls)).toMatchInlineSnapshot(`
      ...
      subscription operation with method PATCH for tenant cf528063-6a43-4bf2-8ccc-ca4e6d75d88e finished with state SUCCEEDED
      subscription operation with method PATCH for tenant 6ef2372b-d256-4fef-8b01-6eeb18f2eefe finished with state SUCCEEDED
      subscription operation with method PATCH for tenant 34d6259c-41bc-4f6b-8220-018ace187813 finished with state SUCCEEDED
